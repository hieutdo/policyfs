...
// TestDoctor_Text_FailListsAllIssues verifies `pfs doctor` (text) lists all discovered issues and prints a summary line.
...
`)
	...
	require.Contains(t, stdout, "config: mount \"media\": mountpoint is required\n")
	require.Contains(t, stdout, "config: mount \"media\": storage_paths must not be empty\n")
	require.Contains(t, stdout, "config: mount \"media\": routing_rules must not be empty\n")
